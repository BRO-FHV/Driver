...
static iBOOL move_window( /* TRUE: successful, FALSE: failed */
...
DWORD clust /* Cluster# to get the link information */
	...
	if (clust >= 2 && clust < fs->max_clust) { /* Valid cluster# */
		...
		case FS_FAT12:
			bc = (WORD) clust * 3 / 2;
			if (!move_window(fs, fatsect + (bc / S_SIZ)))
				break;
			wc = fs->win[bc & (S_SIZ - 1)];
			bc++;
			if (!move_window(fs, fatsect + (bc / S_SIZ)))
				break;
		...
		case FS_FAT16:
			if (!move_window(fs, fatsect + (clust / (S_SIZ / 2))))
				break;
			return LD_WORD(&fs->win[((WORD)clust * 2) & (S_SIZ - 1)]) ;

		case FS_FAT32:
			if (!move_window(fs, fatsect + (clust / (S_SIZ / 4))))
				break;
			return LD_DWORD(&fs->win[((WORD)clust * 4) & (S_SIZ - 1)])
					& 0x0FFFFFFF;
		}
	}
...
#if !_FS_READONLY
static iBOOL put_cluster( /* TRUE: successful, FALSE: failed */
FATFS *fs, /* File system object */
...
DWORD val /* New value to mark the cluster */
	...
	case FS_FAT12:
		...
		*p = (clust & 1) ? ((*p & 0x0F) | ((BYTE) val << 4)) : (BYTE) val;
		...
		p = &fs->win[bc & (S_SIZ - 1)];
		*p = (clust & 1) ?
				(BYTE) (val >> 4) : ((*p & 0xF0) | ((BYTE) (val >> 8) & 0x0F));
		break;
...
#if !_FS_READONLY
static iBOOL remove_chain( /* TRUE: successful, FALSE: failed */
FATFS *fs, /* File system object */
...
#if !_FS_READONLY
static DWORD create_chain( /* 0: no free cluster, 1: error, >=2: new cluster number */
FATFS *fs, /* File system object */
...
static iBOOL next_dir_entry( /* TRUE: successful, FALSE: could not move next */
DIR *dirobj /* Pointer to directory object */
	...
	if ((idx & ((S_SIZ - 1) / 32)) == 0) { /* Table sector changed? */
		...
		if (!dirobj->clust) { /* In static table */
			if (idx >= fs->n_rootdir)
				return iFALSE; /* Reached to end of table */
...
static FRESULT trace_path( /* FR_OK(0): successful, !=0: error code */
...
#if !_FS_READONLY
static FRESULT reserve_direntry( /* FR_OK: successful, FR_DENIED: no free entry, FR_RW_ERROR: a disk error occured */
...
static BYTE check_fs( /* 0:The FAT boot record, 1:Valid boot record but not an FAT, 2:Not a boot record or error */
FATFS *fs, /* File system object */
...
BYTE chk_wp /* !=0: Check media write protection for wrinting fuctions */
	...
	*rfs = fs; /* Returen pointer to the corresponding file system object */
	...
	fs->max_clust = maxclust =
			(totalsect /* Last cluster# + 1 */
			- LD_WORD(&fs->win[BPB_RsvdSecCnt]) - fatsize
					- fs->n_rootdir / (S_SIZ / 32)) / fs->sects_clust + 2;
...
static FRESULT validate( /* FR_OK(0): The object is valid, !=0: Not valid */
...
BYTE mode /* Access mode and file open mode flags */
	...
	if (mode & (FA_CREATE_ALWAYS | FA_OPEN_ALWAYS | FA_CREATE_NEW)) {
		...
		if (mode & FA_CREATE_ALWAYS) {
			...
			ps = get_fattime();
			ST_DWORD(&dir[DIR_WrtTime], ps);
			/* Updated time */
			ST_DWORD(&dir[DIR_CrtTime], ps);
			/* Created time */
			fs->winflag = 1;
		}
	}
...
WORD *br /* Pointer to number of bytes read */
	...
	*br = 0;
	...
	if (btr > remain)
		btr = (WORD) remain; /* Truncate read count by number of bytes left */
...
FRESULT f_sync(FIL *fp /* Pointer to the file object */
	...
	if (res == FR_OK) {
		if (fp->flag & FA__WRITTEN) { /* Has the file been written? */
			...
			/* Update file size */
			ST_WORD(&dir[DIR_FstClusLO], fp->org_clust);
...
FRESULT f_mkdir(const char *path /* Pointer to the directory path */
	...
	ST_DWORD(&dir[DIR_WrtTime], tim);
	/* Crated time */
	ST_WORD(&dir[DIR_FstClusLO], dclust);
	/* Table start cluster */
...
const char *path_new /* Pointer to the new name */
	...
	if (!move_window(fs, sect_old))
		return FR_RW_ERROR; /* Remove old entry */
